...
最简单的部署方式是使用 Docker。该镜像同时包含了前端界面和后端数据库服务。
...
运行以下命令启动服务，并将数据持久化保存到当前目录的 `data` 文件夹中。
...
*   **数据持久化**: 所有的数据库文件 (`invest_track.db`) 将会保存在宿主机的 `data/` 目录中，删除容器不会丢失数据。

### 3. NAS 部署 (群晖/威联通)

1.  将项目代码上传到 NAS。
2.  在 NAS 的 Docker 管理界面中选择“构建镜像”或通过 SSH 执行上述 build 命令。
3.  创建容器时：
    *   **端口映射**: 容器 3001 -> 本地 3001 (或任意空闲端口)。
    *   **存储空间**: 将 NAS 上的某个文件夹（例如 `/docker/invest-track/data`）挂载到容器的 `/app/data`。
...
node server.js
```
*   服务启动在 `http://localhost:3001`，并自动创建 `data/invest_track.db`。
